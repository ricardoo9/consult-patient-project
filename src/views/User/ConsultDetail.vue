...
const onCopy = async () => {
  if (!isSupported.value) showToast('未授权复制命令')
...
<template>
  <div class="consult-detail-page" v-if="item">
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultPay">
      ...
      <van-button type="default" round :loading="loading" @click="cancelOrder(item!)"
        >取消问诊</van-button
      >
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultCancel">
      <van-button type="default" round :loading="deleteLoading" @click="deleteOrder(item!)"
        >删除订单</van-button
      >
